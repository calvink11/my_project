...
def api_get_cate_topword(request):
    ...
    chart_data, wf_pairs = get_category_topword_db(cate, topk)
...
def get_category_topword_db(cate, topk=10):
    #wf_pairs = data[cate][0:topk]
    statement = "SELECT top_keys FROM topkeyword where category='{}'".format(cate)
...
# read df
#df_topkey = pd.read_csv('app_top_keyword/dataset/now_news_topkeys_with_category_via_token_pos.csv', sep=',')
