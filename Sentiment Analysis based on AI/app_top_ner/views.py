...
# category names
news_categories=['焦點','要聞','即時','運動','新奇','財經','娛樂','生活','全球','專題','全部']
...
def load_data_topNer():
    # read data
    df_data= pd.read_csv('app_top_ner/dataset/now_news_topkey_by_ner_and_category.csv')
    ...
    for idx in range(len(df_data)):
        ...
        for cate, wf in eval(df_data.iloc[idx, 1]):
            cate_wf[cate] = wf
        # for each ner 
